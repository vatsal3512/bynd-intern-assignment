...
The output for Bharat Forge is genuinely good. It covers the Kalyani Group connection, the global manufacturing footprint across five countries, the BFL 2.0 strategy, and the market cap. Every sentence has a real URL attached to it, so one can verify the authenticity of the every part of the document.

The financial table is the strongest part. Four years of clean data, correct units (₹ Crores), correct currency detection, and the EPS is handled separately so it does not get wrongly converted. The highlights below the table, revenue growth of 36.8% over three years, net income doubling, EBITDA margin improving from 14.9% to 17.6% all are derived from the actual numbers in the table.
...
The clients section is also extensively covered. It found Boeing, Embraer, Honeywell, General Atomics in aerospace. Daimler, Volkswagen, Toyota, Maruti, Hyundai, Mahindra in automotive. AAM, Dana, Meritor, Valeo in Tier 1 suppliers. Indian Army and Navy with specific contract values. Every client has a source URL.
